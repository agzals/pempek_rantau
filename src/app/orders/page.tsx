        ...
        <tbody>
          {data.map((item: OrderType) => (
            <tr className={`${item.status !== "delivered" && "bg-red-50"}`} key={item.id}>
              ...
              <td className="hidden md:block py-6 px-1">{item.products.length > 0 ? item.products[0].title : "No Product"}</td>
              {session?.user.isAdmin ? (
                <td>
                  <form className="flex items-center justify-center gap-4" onSubmit={(e) => handleUpdate(e, item.id)}>
                    <input placeholder={item.status.toString()} className="p-2 ring-1 ring-red-100 rounded-md" />
